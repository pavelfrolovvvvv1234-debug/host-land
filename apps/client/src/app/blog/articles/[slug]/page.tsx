...
export default function BlogArticlePage({ params }: PageProps) {
  ...
  // In Next.js, process.cwd() returns the project root (where package.json is)
  const projectRoot = process.cwd();
  let filePath = join(projectRoot, "blog", "articles", `${slug}.mdx`);
  let fileContent: string | null = null;

  if (existsSync(filePath)) {
    fileContent = readFileSync(filePath, "utf-8");
  } else {
    filePath = join(projectRoot, "blog", "articles", `${slug}.md`);
    if (existsSync(filePath)) {
      fileContent = readFileSync(filePath, "utf-8");
    } else {
      console.error(`Blog article not found: ${filePath}`);
      notFound();
    }
  }
