...
export default function WikiArticlePage({ params }: PageProps) {
  ...
  const components = {
    a: ({ href, children, ...props }: any) => {
      ...
      if (href && href.startsWith("/ru/wiki/articles/")) {
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const pathname = `/ru/wiki/articles/${slug}`;
  return {
    title: `${title} | База знаний | Dior Host`,
    description: description || `Статья базы знаний: ${title}`,
    alternates: {
