...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full">
      <nav
        className="relative border-b border-white/[0.06] bg-[#030304]/80 backdrop-blur-xl"
      ...
      >
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex min-w-0 shrink-0 items-center">
            <Link
              href={isRu ? "/ru" : "/"}
              className="flex items-center gap-2 transition-opacity hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2 focus:ring-offset-[#080808] rounded-lg"
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              type="button"
              onClick={toggleMenu}
              className="flex h-9 w-9 items-center justify-center rounded-lg text-white/70 hover:bg-white/10 hover:text-white md:hidden"
              aria-label={isMenuOpen ? (isRu ? "Закрыть меню" : "Close menu") : (isRu ? "Открыть меню" : "Open menu")}
              aria-expanded={isMenuOpen}
            >
              {isMenuOpen ? (
                <svg className="size-5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" aria-hidden>
